...
class Expert::LocationsController < ApplicationController
  ...
  def show
    ...
    @experts = User.with_expertise_location(@location.id).exid_holder.not_unavailable.order("users.last_active_at DESC").limit(5)
  ...
  def experts
    @location = Location.find(params[:id])
    @experts = User.with_expertise_location(@location.id).exid_holder.not_unavailable.page(params[:page]).order("users.last_active_at DESC")
    @expert_total_count = User.with_expertise_location(@location.id).exid_holder.not_unavailable.count
  ...
  def experts_email_csv
    ...
    @experts = User.with_expertise_location(@location.id).exid_holder.not_unavailable.order("users.last_active_at DESC")
