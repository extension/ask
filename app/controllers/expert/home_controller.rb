...
class Expert::HomeController < ApplicationController
  ...
  def users_by_tag
    ...
    @experts = User.tagged_with(@tag.id).page(params[:page]).exid_holder.not_unavailable.order("users.last_activity_at DESC")
  ...
  def users_by_county
    ...
    @experts = User.with_expertise_county(@county.id).exid_holder.not_unavailable.page(params[:page]).order("users.last_activity_at DESC")
  ...
  def county
    ...
    @question_total_count = Question.where("county_id = ?", @county.id).not_rejected.count
    @experts = User.with_expertise_county(@county.id).exid_holder.not_unavailable.order("users.last_activity_at DESC").limit(5)
    @expert_total_count = User.with_expertise_county(@county.id).exid_holder.not_unavailable.count
