class Tag < ActiveRecord::Base
  has_many :taggings
  scope :used_at_least_once, includes(:taggings).group("taggings.tag_id").having("COUNT(taggings.id) > 0")
  scope :not_used, includes(:taggings).group("taggings.tag_id").having("COUNT(taggings.id) = 0")
