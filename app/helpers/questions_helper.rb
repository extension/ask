module QuestionsHelper
  def stringify_question_event(q_event)
    ...
    when QuestionEvent::EXPERT_EDIT_RESPONSE
      return "Question response edited by expert <strong #{qw}>#{initiator_full_name}</strong> <span>#{time_ago_in_words(q_event.created_at)} ago</span> <small>#{humane_date(q_event.created_at)}</small> see individual response history for more information".html_safe
